...
x <- runif(30)
test1 <- seatsVotes(x)
test2 <- seatsVotes(1-x)

#plot(test1)
#par(new=TRUE)
#plot(test2, legend = FALSE)
...
#plot.seatsVotes2(y1, col = "blue")
#par(new=TRUE)
#plot.seatsVotes2(y2, col ="red")

```


```{r}
mn <- read_csv("/Users/evaairoldi/Desktop/comps/test_MN_data.csv")

mn_work <- cbind("Map" = c(1:nrow(mn)), mn) 

mn_tidy <- mn_work %>% pivot_longer(!c(Map, efficiency_gap, mean_median, seats_won),
                         names_to = c(".value", "District"),
                         names_sep = -1) 

m1 <- mn_tidy %>% filter(Map == 1)

demm1 <- seatsVotes(m1[8])
plot.seatsVotes(demm1)
```  
